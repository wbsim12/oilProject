...
#
#         # 초기에 한번 실행하여 텍스트 설정
#         self.show_all_prices()
#         self.show_local_prices()
class show_prices():
    def show_all_prices(self):
        ...
        return all_gasoline_price, all_gasoline_diff, all_diesel_price, all_diesel_diff
        # 각각의 라벨에 평균 가격과 DIFF 정보 설정
        # all_gasoline_label.setText("휘발유 {:.2f}원 (전일대비 {:+.2f}원)".format(all_gasoline_price, all_gasoline_diff))
        # all_diesel_label.setText("경유 {:.2f}원 (전일대비 {:+.2f}원)".format(all_diesel_price, all_diesel_diff))

    def show_local_prices(self):
        ...
        return local_gasoline_price, local_gasoline_diff, local_diesel_price, local_diesel_diff
        # 각각의 라벨에 평균 가격과 DIFF 정보 설정
